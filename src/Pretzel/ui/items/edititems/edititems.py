...
from Pretzel.constants import AddItemsDialogType
from Pretzel.core.converter import list_to_string
from Pretzel.core.database.items import edit_items
from Pretzel.core.models import ItemsModel, PictogramModel
from Pretzel.ui.dialogs import AddItemsDialog, AddPictogramsDialog
...
class EditItems(QDockWidget):
    ...
    def setup_ui(self):
        uic.loadUi("Pretzel/ui/items/edititems/edititems.ui", self)
    ...
    def bind_signals(self):
        ...
        self.save_items_button.clicked.connect(self.save_items)
    ...
    @pyqtSlot()
    def add_item(self):
        add_items_dialog = AddItemsDialog(parent=self, type_=AddItemsDialogType.Item)
        add_items_dialog.exec()

        for item in self.items_model.items:
            item["Old Name"] = item["Name"]
        ...
        # If you quit without selecting an item, Pretzel throws an error
        try:
            self.entry_name.setText(self.items_model.items[self.items_list.currentIndex().row()]["Name"])
            self.pictograms_list.setModel(self.items_model.items[self.items_list.currentIndex().row()]["Pictograms"])
        except IndexError:
            pass
    ...
    def save_items(self):
        ...
        for item_index in range(self.items_model.rowCount()):
            old_name = self.items_model.items[item_index]["Old Name"]
            ...
            items.append((old_name, name, chem_formula, warning_label, danger_level, notes, pictograms))
