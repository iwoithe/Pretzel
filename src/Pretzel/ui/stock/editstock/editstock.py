...
from Pretzel.core.database.stock import edit_stock
from Pretzel.core.models import StockModel, StockListModel


class EditStock(QDockWidget):
    ...
    def setup_ui(self):
        uic.loadUi("Pretzel/ui/stock/editstock/editstock.ui", self)

        self.previous_index = None

        # Add the units
        self.units = ["None", "Millilitres", "Litres", "Grams", "Kilograms", "Pounds"]
        self.combo_unit.addItems(self.units)
    ...
    def save_stock_settings(self, item_index):
        ...
        try:
            ...
            settings["Unit"] = self.units[self.combo_unit.currentIndex()]
            settings["Cost"] = self.spin_cost.value()
    ...
    def update_stock_display(self, item_index):
        ...
        try:
            ...
            self.combo_unit.setCurrentIndex(self.units.index(settings["Unit"]))
            self.spin_cost.setValue(settings["Cost"])
        except:
            pass
    ...
    @pyqtSlot()
    def add_stock(self):
        add_items_dialog = AddItemsDialog(parent=self, type_=AddItemsDialogType.Stock, stock_type_=StockType.Edit)
    ...
    def save_stock(self):
        ...
        for item_index in range(self.stock_model.rowCount()):
            ...
            unit = self.stock_model.stock[item_index]["Unit"]
            cost = self.stock_model.stock[item_index]["Cost"]

            items.append((name, quantity, unit, cost))
